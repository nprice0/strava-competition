...
### Manual exchange with an existing authorisation code

If the Strava user has already completed the authorisation step and sent you the one-time `code` (along with your app’s client credentials), you can swap it for tokens without re-running the helper script. Run the following curl command, replacing the placeholders with real values:

```bash
curl -X POST https://www.strava.com/oauth/token \
  -d client_id=<your_client_id> \
  -d client_secret=<your_client_secret> \
  -d code=<authorisation_code_from_user> \
  -d grant_type=authorization_code
```

The response JSON contains both `access_token` (short-lived) and `refresh_token` (long-lived). Paste the `refresh_token` into the `Runners` sheet for that athlete. Keep the refresh token secure; it allows ongoing access until revoked.
