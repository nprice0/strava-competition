            ...
            for segment_name, team_data in results.items():
                ...
                if rows:
                    ...
                    if "Fastest Time (sec)" in df.columns:
                        ...
                        # Overall rank across the whole segment (fastest = 1); ties share rank (competition ranking)
                        df["Rank"] = df["Fastest Time (sec)"].rank(method="min", ascending=True).astype(int)
                        # Per-team rank (useful when viewing by team); ties share rank
                        if "Team" in df.columns:
                            df["Team Rank"] = df.groupby("Team")["Fastest Time (sec)"].rank(method="min", ascending=True).astype(int)
